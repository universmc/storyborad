...
response=$(curl -s -X POST https://api.groq.com/openai/v1/chat/completions \
  ...
  -d "{
    \"messages\": [
      ...
      {
        \"role\": \"user\",
        \"content\": \"ta réponse doit intégralement être rédigé au format SOLIDITY En intégrant le ${SCRIPT} pour monetiser le ${CONTEXT} et le script du ${SMARTCONTRACT}, En respectant les normes du Web sémantique W3C output.sol\"
      }
    ],
...
sol_content=$(echo "$response" | jq -r '.choices[0].message.content')

# Vérifier si le contenu HTML est récupéré
if [ -z "$sol_content" ]; then
...
timestamp=$(date +%s)
output_file="smart_${timestamp}.sol"

# Écriture du contenu dans le fichier
echo "$sol_content" > "$output_file"
