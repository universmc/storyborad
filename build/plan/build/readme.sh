...
# Configuration
CONTEXT="readme: développement d'une interfaces de configuration"
JSON="[]"
SCRIPT="[]"
...
response=$(curl -s -X POST https://api.groq.com/openai/v1/chat/completions \
  ...
  -d "{
    \"messages\": [
      ...
      {
        \"role\": \"user\",
        \"content\": \"ta réponse doit intégralement être rédigé au format Mardown En intégrant le ${SCRIPT} pour afficher le ${CONTEXT}.json , En respectant les normes du Web sémantique W3C\"
      }
    ],
...
md_content=$(echo "$response" | jq -r '.choices[0].message.content')
...
timestamp=$(date +%s)
output_file="readme_${timestamp}.md"
